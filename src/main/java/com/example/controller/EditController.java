...
public class EditController {
	...
	public String showEditPage(Model model, ItemForm form, Integer itemId,BindingResult br) {
		...
		if(!br.hasErrors()) {
			...
			form.setParentId(item.getCategoryList().get(PARENT_LEVEL).getId().toString());
			form.setChildId(item.getCategoryList().get(CHILD_LEVEL).getId().toString());
			form.setGrandChildId(item.getCategoryList().get(GRAND_CHILD_LEVEL).getId().toString());
